...
insert into products (name, brand, category_id, unit) values
  ('Прясно мляко 3.5%', 'Верея', (select id from categories where name='Мляко и млечни'), 'л'),
  ('Кашкавал Витоша', 'Млечна слава', (select id from categories where name='Мляко и млечни'), 'кг'),
  ('Масло 125г', 'Президент', (select id from categories where name='Мляко и млечни'), 'бр.'),
  ('Пилешки бутчета', NULL, (select id from categories where name='Месо и колбаси'), 'кг'),
  ('Кайма смесена', NULL, (select id from categories where name='Месо и колбаси'), 'кг'),
  ('Шунка варена', 'Стара Загора', (select id from categories where name='Месо и колбаси'), 'кг'),
  ('Домати', NULL, (select id from categories where name='Плодове и зеленчуци'), 'кг'),
  ('Краставици', NULL, (select id from categories where name='Плодове и зеленчуци'), 'кг'),
  ('Ябълки Голдън', NULL, (select id from categories where name='Плодове и зеленчуци'), 'кг'),
  ('Хляб Добруджа', 'Пекарна', (select id from categories where name='Хляб и хлебни'), 'бр.'),
  ('Вода Горна Баня 1.5л', 'Горна Баня', (select id from categories where name='Напитки'), 'бр.'),
  ('Coca-Cola 2л', 'Coca-Cola', (select id from categories where name='Напитки'), 'бр.'),
  ('Перилен препарат Ariel 3кг', 'Ariel', (select id from categories where name='Почистващи'), 'бр.'),
  ('Пица Маргарита', 'Dr. Oetker', (select id from categories where name='Замразени'), 'бр.'),
  ('Риба тон в масло', 'Rio Mare', (select id from categories where name='Консерви'), 'бр.');
...
-- Default списък за пазаруване
insert into shopping_lists (name, budget_eur) values ('Седмично пазаруване', 100);
